...
Ideally, we want a short certificate, so ECDSA p256 is chosen as the signature algorithm. Additionally, modern TLS implementations use the `serverAltName` extension (SAN) for host verification instead of common names (see [[rfc6125]](www.rfc-editor.org/info/rfc6125)), so we have to place our node identifiers there. Fortunately we can use non-existent but valid domain names in SAN.
